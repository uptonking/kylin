...
if [ -z "$(command -v mvn)" ]
then
    echo "Please install maven first so that Kylin packaging can proceed" &&     exit 1
else
...
if [ -z "$(command -v git)" ]
then
    echo "Please install git first so that Kylin packaging can proceed" &&     exit 1
else
...
if [ -z "$(command -v npm)" ]
then
    echo "Please install npm first so that Kylin packaging can proceed" &&     exit 1
else
...
echo "kylin version: ${version}" && export version
